...
| 规则                   | 原生 CSS 嵌套              | Sass/SCSS |
| ---------------------- | -------------------------- | --------- |
| 父级引用必须显式写 `&` | 必须                       | 必须      |
| 可以省略 `&` 的情况    | 可以直接写选择器（新特性） | 不行      |
| @规则嵌套（如 @media） | 支持直接嵌套               | 支持      |
| 嵌套列表项             | 不支持用逗号分隔多个父级   | 支持      |
...
2025 年的 CSS 原生嵌套已经非常成熟，日常开发中：

- 普通后代选择器直接写（不用&，最爽）
- 伪类、伪元素、组合选择器、父级加类时必须加 `&`
- @media、@supports 等可以直接嵌套
