...
### Flex和Grid中justify和align的区别

| 属性              | 作用对象      | Flex (一维)                  | Grid (二维)                                       |
| ----------------- | ------------- | ---------------------------- | ------------------------------------------------- |
| `justify-content` | 容器整体      | 控制主轴（横向）剩余空间分配 | 控制所有格子整体在容器里的水平位置                |
| `justify-items`   | 单元格内元素  | 无效                         | 控制每个格子里的内容水平对齐                      |
| `align-content`   | 多行整体      | 仅在 flex-wrap: wrap         | 且有多行时有效 控制所有格子整体在容器里的垂直位置 |
| `align-items`     | 单行/格内元素 | 控制当前行内元素的垂直对齐   | 控制每个格子里的内容垂直对齐                      |

**Flex 布局：简单直接**
在 Flex 中，你几乎只需要关心 `justify-content`（左右）和 `align-items`（上下）。

- `justify-content`: 决定一排元素是挤在左边、右边，还是平分空间（space-between）。
- `align-items`: 决定这一排元素在高度上是怎么对齐的（顶对齐、居中、底对齐）。
- 注意：`justify-items` 在 Flex 里是报错/无效的。

**Grid 布局：精细控制**
Grid 像是在一个容器里画好了很多“小抽屉”。

- `justify-content` / `align-content`: 如果你的格子总大小小于容器（比如容器 500px，格子加起来才 300px），这两个属性决定整组格子靠哪边站。
- `justify-items` / `align-items`: 决定抽屉里的东西怎么摆。默认是 stretch（撑满格子），你也可以改成 center 让内容在格子里居中。
