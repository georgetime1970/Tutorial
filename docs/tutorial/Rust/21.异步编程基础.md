...
捕获外部环境并返回一个实现了 `Future` trait 的匿名类型。
...
- 编译器将 `async/await` 代码转换为实现了 `Future` trait 的等效代码，类似 `for` 循环被转换为使用 `Iterator` trait 的代码。
...
```rust
fn main() {
    // trpl::block_on() 中只能有一个 future,也就是一个 async 块或 async fn 调用,但是这个 future 内部可以包含任意数量的 await 调用
    trpl::block_on(async {
        async_main().await;
    });
}
...
fn main() {
    trpl::block_on(async {
        // spawn_task 会创建一个新的任务并立即激活它,同样需要主动阻塞当前任务,让 spawn_task 生成的任务有机会运行
...
fn main() {
    trpl::block_on(async {
        ...
        handle.await.unwrap(); // 等待 spawn_task 任务完成
    });
}
```
...
- `send` 和 `recv` 都是异步方法，需要使用 `.await` 来等待它们完成。
...
fn main() {
    trpl::block_on(async {
        let (tx, mut rx) = channel::<u32>(); // 创建一个异步通道，发送者 tx 和接收者 rx
