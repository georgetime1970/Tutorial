...
- `trpl::spawn_task` 并不是立即执行,依旧需要运行时调度,让 `spawn_task` 生成的任务有机会运行。4cv bgrtfcrv4 7
...
// trpl::spawn_task 并发上等同于,但底层机制不同
trpl::block_on(async {
...
- `recv` 是异步方法，需要使用 `.await` 来等待它们完成。`send` 方法不需要 `.await`，也不会阻塞程序。
