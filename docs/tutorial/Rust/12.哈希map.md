...
- 覆盖: 由于`HashMap`的键是唯一的,只要重复插入相同键就可以覆盖原值
- 不存在就插入: `entry(k).or_insert(v)`方法,entry(k)返回枚举
...
let mut scores = HashMap::new();
scores.insert(String::from("Blue"), 10);
scores.entry(String::from("Yellow")).or_insert(50); // 不存在Yellow就插入,值为50
scores.entry(String::from("Blue")).or_insert(50); // blue存在,不会插入
println!("{scores:?}");
```
