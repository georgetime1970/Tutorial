...
| 宏类型                | 语法特征             | 常见用途                                             |
| --------------------- | -------------------- | ---------------------------------------------------- |
| 自定义 `#[derive]` 宏 | `#[derive(MyMacro)]` | 自动为结构体或枚举实现特定的 Trait（如 `Serialize`） |
...
定义过程宏需要放在一个**独立的 crate** 中，且该 crate 的类型必须设置为 `proc-macro`。这是因为过程宏需要特殊的编译环境，必须与普通 Rust 代码分开编译。
...
>
> `TokenStream` 代表 Rust 代码的 Token 序列，比普通字符串具有更丰富的结构，可用于分析和操作代码。

#### 过程宏的三大类型

| 注解                      | 类型名称                  | 用法示例                | 作用描述                                                                                 |
| ------------------------- | ------------------------- | ----------------------- | ---------------------------------------------------------------------------------------- |
| `#[proc_macro_derive]`    | 派生宏（Derive）          | `#[derive(HelloMacro)]` | 最常用的过程宏。为结构体或枚举自动实现 Trait。**只会追加代码，不修改原定义。**           |
| `#[proc_macro_attribute]` | 属性宏（Attribute-like）  | `#[route(GET, "/")]`    | 类似"装饰器"。可附着在任何项上（函数、结构体等），**可修改或替换原代码**，并可接收参数。 |
| `#[proc_macro]`           | 类函数宏（Function-like） | `sql!(...)`             | 使用方式类似函数调用（带 `!`）。接收一段代码并返回生成的代码，语法灵活可自定义。         |

### 自定义 derive 宏

derive 宏是最常见的过程宏类型，允许你为结构体或枚举**自动生成 Trait 的实现**。通过 `#[derive(MyMacro)]` 语法，编译器会在编译时为你的类型生成对应代码，可大量减少样板代码。
...
4. `.into()` 将 `quote!` 生成的结果转换回 `TokenStream` 供编译器使用。
...
类属性宏（Attribute-like Macro）与 derive 宏相似，但更为灵活：

- derive 宏**只能**用于结构体和枚举，且只追加代码。
...
类函数宏（Function-like Macro）的调用语法与普通宏相同（使用 `!`），但其底层实现是过程宏，因此比 `macro_rules!` 更灵活，可以执行任意复杂的代码分析与生成。
