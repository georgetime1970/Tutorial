...
- 更新语法必须在新结构体的后面
- 更新语法会转移结构体字段的所有权
- 部分移动规则：非 `Copy` 字段被移动后，整个结构体实例进入不完整状态，无法访问任何字段
...
- 部分移动规则：一旦非 `Copy` 字段被移动，原变量进入"不完整"状态，无法访问任何字段（包括 `Copy` 字段）

> **部分移动（Partial Move）**: 结构体中有非 `Copy` 字段（如 `String`）被移动走后，Rust 编译器认为原变量处于"不完整"状态，不允许再访问该变量的任何字段。这是 Rust 的安全机制。
>
