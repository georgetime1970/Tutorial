...
- `impl Enum` 中，函数的第一个参数可以是 `self`，也可以是 `&self` 或 `&mut self`，或者没有 `self` 参数（静态方法）。
...
fn main() {
    let home = IpAddr::V4(127, 0, 0, 1);
    home.call(); // 使用关联函数中的方法
    home.call_mut(); // 使用关联函数中的方法
...
- `impl Enum` 中，函数没有 `self` 参数，这样的函数叫做关联函数（associated function），通常用来定义构造函数。类似于面向对象语言中的静态方法（static method）。
- 一般用于返回一个枚举新实例的构造函数
