...
> 在 [match模式匹配](./8.match模式匹配.md)中我们会详细介绍`match`等语法的模式匹配,这里先简单使用一下
...
- `unwrap()`: 如果是 `Some(值)`,返回值；如果是 `None`,直接 panic
- `unwrap_or(default)`: 如果是 `None`,返回默认值
- `unwrap_or_else(|| default)`: 如果是 `None`,执行闭包生成默认值(延迟求值)
- `expect("msg")`: 和 `unwrap` 一样,但 panic 时显示自定义消息,便于调试
- `map(|x| x + 1)`: 如果是 `Some`,对值进行转换；如果是 `None`,保持 `None`,返回新的 `Option`
- `filter(|x| x > &0)`: 如果是 `Some` 且条件满足,保持 `Some`；否则返回 `None`
- `and_then(|x| Some(x + 1))`: 链式处理 `Option`,闭包返回 `Option`
- `?`: 如果有值就继续,如果是 `None`,直接从函数返回 `None`(仅用于返回 `Option` 或 `Result` 的函数)
