...
**Deref 强制转换**(Deref coercions)是 Rust 的一个便利功能,它在编译时自动将引用从一种类型转换为另一种类型,前提是目标类型实现了 `Deref trait`.这使得在函数和方法调用中使用智能指针变得非常方便,因为它们会自动被转换为常规引用.
...
**Deref 隐式转换**(Deref coercions)是 Rust 的一个便利功能,它在编译时自动将引用从一种类型转换为另一种类型,前提是目标类型实现了 `Deref trait`.这使得在函数和方法调用中使用智能指针变得非常方便,因为它们会自动被转换为常规引用.
...
3. 当我们传递 `&m` 给 `hello` 时,发生了 **Deref 隐式转换**:
   - `&MyBox<String>` 自动转换为 `&String`,因为 `MyBox` 实现了 `Deref<Target=String>`,(`&`触发了 `Deref` 转换)
   - `&String` 自动转换为 `&str`,因为 `String` 实现了 `Deref<Target=str>`.
...
使用 Box 之后,编译器知道 `Cons` 变体中的 `Box<List>` 是一个指针,指向堆上的另一个 `List` 值,而不是直接包含另一个 `List` 值,从而解决了递归类型无限大小的问题.
...
fn main() {
    let a: Rc<Node> = Rc::new(Node { next: RefCell::new(None) });
    let b: Rc<Node> = Rc::new(Node { next: RefCell::new(None) });

    // 1. 让 a 指向 b (强引用)
    // 注意：这里为了展示打破循环，通常只在一端使用 Weak
    // 2. 让 b 指向 a (弱引用)，使用 Rc::downgrade
    *b.next.borrow_mut() = Some(Rc::downgrade(&a));

    // 现在：
    // a 的强引用计数 = 1 (只有变量 a)
    // a 的弱引用计数 = 1 (来自 b 的 Weak)
    // 当 main 结束，a 离开作用域，强引用归零，a 被成功释放！
    // a 释放后，b 的强引用也归零，b 随之被释放。

    // --- 使用 .upgrade() 的地方 ---
    // 1. 获取 b 内部存的那个弱引用
    let borrow: Ref<Option<Weak<Node>>> = b.next.borrow();
    if let Some(weak_ref) = borrow.as_ref() {
        // 2. 尝试升级：由于 a 还活着（main 函数没结束），这里会成功
        match weak_ref.upgrade() {
            Some(strong_a) => {
                println!("成功访问到节点 a！它的 ID 是: {}", strong_a.id);
            }
            None => {
                println!("节点 a 已经被销毁了。");
            }
