...
// 创建两个彼此互相指向的 List 值，从而形成引用循环,引用计数永远不会归零 → 内存泄漏
fn main() {
...
- 通过 `Rc::downgrade(&rc)` 创建弱引用,通过 `.upgrade()` 尝试获取强引用(返回 `Option<Rc<T>>`).
...
struct Node {
    value: i32,
    parent: RefCell<Weak<Node>>,   // 父节点: 用弱引用,避免循环
    children: RefCell<Vec<Rc<Node>>>, // 子节点: 用强引用
}
...
fn main() {
    ...
    // leaf 的父节点指向 branch(弱引用,不增加 branch 的强引用计数)
