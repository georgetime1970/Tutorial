...
- 查看内存占用: `free -h`
- 列出硬盘信息: `lsblk`
- 查看磁盘占用: `df -h`, `-h `(human)参数会以人类易读的格式显示（比如 GB 或 MB）
...
- 发行系统版本: `cat /etc/os-release`

- 查看本机名: `hostname`

- 临时设置主机名(改的内存): `hostname NAME`

- 永久设置主机名: `nano /etc/hostname` 或 `hostnamectl set-hostname NAME`, 修改文件需要重启才能生效,推荐用后者

- 显示系统运行时间和负载: `uptime`

- 查看本机 ip(内网): `hostname -I`
...
- 切换用户: `sudo -i`, sudo 用于提权

- 显示当前用户名: `whoami`

- 登录前提示: `nano /etc/issue`
...
- 注销: `logout` 或 `exit`或`^+d`

- 关机或重启: `shutdown`, 计划性关机，允许延迟，通知用户 [参考](https://www.runoob.com/linux/linux-comm-shutdown.html)
...
- 判断是否是 bash 内部命令: `type`

  - 内部命令不显示路径,优先级: 别名 alias>内部命令>外部命令

- 显示当前文件路径: `pwd`

- 当前 shell 执行脚本: `source` 或 `.`
...
- 查看程序手册: `man` 或 `info`(比 man 更详细)

- 程序描述: ``whatis`

- `stat`: 显示文件的状态

- `hexdump`: 以十六进制，十进制，八进制，或 asphalt 显示文本内容
...
### 文件目录结构 [参考](https://www.runoob.com/linux/linux-system-contents.html)
...
### 文件类型 [参考](https://www.runoob.com/linux/linux-file-attr-permission.html)
